...
@Component({
  ...
  template: `
    <div class="p-8 pb-16 space-y-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-2 text-sm text-slate-300">Administra todos los eventos de la plataforma</p>
        </div>
      ...
      >
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium text-white mb-2"> Buscar eventos </label>
            <input
              [(ngModel)]="searchTerm"
              (input)="filterEvents()"
              type="text"
          ...
          <div>
            <label class="block text-sm font-medium text-white mb-2"> Venue </label>
            <select
              [(ngModel)]="venueFilter"
              (change)="filterEvents()"
              class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md shadow-sm text-white focus:outline-none focus:ring-violet-500 focus:border-violet-500"
          ...
          <div class="flex items-center">
            ...
            <div class="ml-3">
              ...
              <p class="text-lg font-semibold text-green-600">
                {{ getEventsByStatus('ACTIVE').length }}
              </p>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-3">
              ...
              <p class="text-lg font-semibold text-red-600">
                {{ getEventsByStatus('CANCELLED').length }}
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <div class="px-8 py-6 border-b border-slate-700/50">
          <h3 class="text-xl font-semibold text-white">Eventos ({{ filteredEvents.length }})</h3>
        </div>
        ...
        <div *ngIf="!loading" class="overflow-x-auto">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-10 w-10">
                      <div
                        class="h-10 w-10 rounded-lg bg-violet-100 flex items-center justify-center"
                      >
                        <svg
          ...
          <div *ngIf="filteredEvents.length === 0 && !loading" class="text-center py-12">
            ...
            <button
              *ngIf="!searchTerm && !statusFilter && !venueFilter"
              (click)="openCreateModal()"
    ...
    >
      <div
        class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-slate-800 border-slate-700"
      >
        ...
        <form (ngSubmit)="saveEvent()" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-white mb-2"> Nombre del Evento * </label>
            <input
          ...
          <div>
            <app-image-upload
              ...
              description="Imagen principal del evento (se mostrará en portada)"
              uploadType="events"
              [multiple]="false"
              [maxFiles]="1"
          ...
          <div>
            <app-image-upload
              ...
              description="Imágenes adicionales (máx. 10)"
              uploadType="events"
              [multiple]="true"
              [maxFiles]="10"
              [existingImages]="eventForm.images || []"
              (imagesUploaded)="onEventImagesUploaded($event)"
              (imageRemoved)="onEventImageRemoved($event)"
            ></app-image-upload>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-white mb-2"> Fecha del Evento * </label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-white mb-2"> Artista Principal * </label>
              <input
                [(ngModel)]="eventForm.headliner"
                name="headliner"
                type="text"
                required
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-white mb-2"> Precio Mínimo (€) * </label>
              <input
                [(ngModel)]="eventForm.minPrice"
                name="minPrice"
                type="number"
                required
                min="0"
            ...
            <div>
              <label class="block text-sm font-medium text-white mb-2"> Precio Máximo (€) * </label>
              <input
                [(ngModel)]="eventForm.maxPrice"
                name="maxPrice"
                type="number"
                required
                min="0"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-white mb-2"> Capacidad Total * </label>
              <input
                [(ngModel)]="eventForm.totalCapacity"
                name="totalCapacity"
                type="number"
                required
                min="1"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-white mb-2"> Estado del Evento </label>
              <select
    ...
    <div
      *ngIf="showLocalitiesModal"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    >
      <div
        class="bg-slate-800 rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden"
      >
        <!-- Header del Modal -->
        ...
        <div class="p-6 overflow-y-auto max-h-[70vh]">
          ...
          <div
            *ngIf="!loadingLocalities && eventLocalities.length > 0"
          ...
          <div *ngIf="!loadingLocalities && eventLocalities.length > 0" class="space-y-4">
            <div
              *ngFor="let locality of eventLocalities"
              ...
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center">
                  <div
                    ...
                    [style.background-color]="locality.color"
                  ></div>
                  <h4 class="text-lg font-medium text-white">{{ locality.name }}</h4>
                </div>
                <span
                  [class]="getLocalityStatusClass(locality)"
                  class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                >
                  ...
                  <div class="space-y-2">
                    <div class="flex justify-between text-sm">
                      ...
                      <span class="text-white font-medium"
                        >{{ 100 - getAvailabilityPercentage(locality) }}%</span
                      >
                    </div>
                    ...
                    <div class="flex justify-between text-sm mt-3">
                      ...
                      <span class="text-white font-bold">{{
                        locality.price | currency : 'EUR' : 'symbol' : '1.2-2'
                      }}</span>
                    </div>
                  </div>
                </div>
...
export class EventsListComponent implements OnInit {
  ...
  saveEvent() {
    ...
    const eventData = {
      ...
      bannerImage: this.eventForm.bannerImage,
      thumbnailImage: this.eventForm.thumbnailImage,
      ...
      this.adminService.updateEvent(this.eventForm.id, eventData).subscribe({
        next: (response: any) => {
          ...
          if (response.ignoredFields && response.ignoredFields.length > 0) {
            message += `\n\nNota: Algunos campos (${response.ignoredFields.join(
              ', '
  ...
  editEvent(event: Event) {
    ...
    this.eventForm = {
      ...
      thumbnailImage: (event as any).thumbnailImage || '',
  ...
  getLocalityStatusClass(locality: any): string {
    ...
    if (locality.availableTickets <= locality.capacity * 0.1)
  ...
  onBannerUploaded(imageUrls: string[]) {
    if (imageUrls.length > 0) {
      this.eventForm.bannerImage = imageUrls[0];
      if (!this.eventForm.thumbnailImage) {
        this.eventForm.thumbnailImage = imageUrls[0];
      }
    }
  }

  onEventImagesUploaded(imageUrls: string[]) {
    if (!this.eventForm.images) {
      this.eventForm.images = [];
    }
    this.eventForm.images = [...this.eventForm.images, ...imageUrls];
  }

  onEventImageRemoved(imageUrl: string) {
    if (this.eventForm.images) {

...
class ReservationController {
    ...
    async createReservation(request, reply) {
        try {
            ...
            logger_1.logger.info(`Reserva creada: ${reservation.id} por usuario VIP ${user.id}`);
    ...
    async getMyReservations(request, reply) {
        try {
            const user = request.user;
            if (!user?.id) {
            ...
            const reservations = await prisma.reservation.findMany({
                where: {
                    userId: user.id,
                    status: { in: [client_1.ReservationStatus.ACTIVE, client_1.ReservationStatus.COMPLETED] }
                },
                include: {
                    event: {
                        select: {
            ...
            const reservationsWithTimeLeft = reservations.map(reservation => {
                const timeLeft = reservation.status === client_1.ReservationStatus.ACTIVE
                    ? Math.max(0, Math.floor((reservation.expiresAt.getTime() - Date.now()) / 1000))
                    : 0;
                return {
                    ...reservation,
                    timeLeftSeconds: timeLeft,
                    isExpired: timeLeft === 0 && reservation.status === client_1.ReservationStatus.ACTIVE
                };
            });
            return reply.send({
                success: true,
    ...
    async cancelReservation(request, reply) {
        try {
            ...
            logger_1.logger.info(`Reserva cancelada: ${id} por usuario ${user.id}`);
            return reply.send({
                success: true,
    ...
    async expireReservations() {
        try {
            ...
            for (const reservation of expiredReservations) {
                ...
                logger_1.logger.info(`Reserva expirada automáticamente: ${reservation.id}`);
                // Publicar evento en RabbitMQ
                const { rabbitmqService } = await Promise.resolve().then(() => __importStar(require('../services/rabbitmq.service')));
            ...
            logger_1.logger.info(`${expiredReservations.length} reservas expiradas y liberadas`);
    ...
    async getAllReservations(request, reply) {
        try {
            const user = request.user;

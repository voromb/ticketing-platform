...
class PaymentController {
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      if (DEMO_MODE) {
        logger.warn('⚠️ MODO DEMO: Usando checkout interno sin Stripe real');
      ...
      return reply.status(500).send({ success: false, error: 'Error al crear sesión de pago', details: error.message });
    }
  }
  ...
  async completeDemoPayment(
    ...
    logger.info('🔥 ENDPOINT /complete-payment LLAMADO');
    try {
      const user = (request as any).user;
      const { orderId } = request.body;

      logger.info(`👤 Usuario: ${user?.id || 'NO AUTENTICADO'}`);
      logger.info(`📋 OrderId recibido: ${orderId || 'NO RECIBIDO'}`);
      ...
      if (!orderId) {
        logger.error('❌ orderId no proporcionado');
        return reply.code(400).send({ success: false, error: 'orderId requerido' });
      }

      logger.info(`💳 DEMO: Iniciando pago para orden ${orderId}`);
      // 1. Obtener la orden
      const order = await prisma.order.findUnique({
        where: { id: orderId },
        include: {
      ...
      logger.info(`📦 Orden encontrada: ${order.quantity} tickets para ${order.event.name}`);
      ...
      logger.info(`🔍 Verificación: Orden ${updatedOrder.id} ahora tiene status: ${updatedOrder.status}`);
      ...
      logger.info(`📊 Stock actualizado: -${order.quantity} disponibles, +${order.quantity} vendidos`);
      ...
      if (order.reservationId) {
        ...
        logger.info(`🔖 Reserva ${order.reservationId} completada y liberada`);
      }
      ...
      logger.info(`🎫 ${tickets.length} tickets generados exitosamente`);
      ...
      logger.error('❌ Error completing demo payment:', error);
    ...
    try {
      ...
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          logger.info(`📤 Evento publicado: payment.completed para orden ${orderId}`);
  ...
  async checkPaymentStatus(
    ...
    try {
      ...
      return reply.status(500).send({ success: false, error: 'Error verificando estado de pago' });
    }
  }
